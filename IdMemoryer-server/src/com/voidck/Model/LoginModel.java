...
public class LoginModel {
    ...
    public static User login(User user){
        String loginAccount = user.getLoginAccount();   //从user取得数据
...
//        int count = 0;
        try{
            ...
            pstmt = con.prepareStatement("select * from userlist where account=? and password=?");    //使用PreparedStatement对象执行SQL语句  count(*)聚集函数:统计元组个数
            pstmt.setString(1,loginAccount);    //设置第一个参数值
            pstmt.setString(2,loginPassword);   //设置第二个参数值
...
//                count = rs.getInt(1);       //返回int类型字段1的字段值
                ...
                rs_user.setLoginAccount(loginAccount);
                rs_user.setLoginPassword(loginPassword);
